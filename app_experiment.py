...
def load_dataset(name: str, random_seed: int):
    ...
    if name.endswith('.data'):
        ...
        # Convert y to numeric and handle different data types
        try:
            y = pd.to_numeric(y, errors='coerce')
            # Convert to binary classification (0, 1) for numeric data
            y = (y > 0).astype(int)
        except:
        ...
        try:
            y = pd.to_numeric(y, errors='coerce')
            # For numeric data, convert to binary if needed
            ...
            le = LabelEncoder()
            y = le.fit_transform(y)
            # Convert to binary if more than 2 classes
            if len(np.unique(y)) > 2:
                y = (y > np.median(y)).astype(int)
...
def create_experiment_id():
    ...
    return f"exp_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"

def create_run_id():
    """Create unique run ID"""
    return f"run_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_{str(uuid.uuid4())[:8]}"
...
with st.sidebar:
    ...
    exp_name = st.text_input("Experiment Name", value=f"Experiment_{datetime.datetime.now().strftime('%Y%m%d_%H%M')}")
...
if run_experiment:
    ...
    for run_idx in range(int(n_runs)):
        status_text.text(f"Running {exp_name} - Run {run_idx+1}/{n_runs}")
